...
### GENERAL 보고서 타입
* `서버 대시보드`의 월간 지표 보고서를 생성합니다.
* 지표는 `CPU평균`, `CPU최대`, `메모리평균`, `메모리최대`, `네트워크 송신`, `네트워크 수신`, `파일시스템 사용률`중 하나 이상을 선택할 수 있습니다.
* 주기는 `5분`, `30분`, `2시간`, `1일`중 하나를 선택할 수 있으며, 선택한 주기에 따라 `월`의 선택이 제한됩니다.(최대 6개월 전 까지 선택 가능)
* `다운로드 요청`버튼을 누르면 보고서를 생성 요청하게 되며 이 목록은 `보고서 생성 요청 현황`에서 확인 가능합니다.
  * 보고서의 생성은 비동기적으로 이루어지므로 생성이 완료될 때 까지는 다운로드할 수 없습니다.

### PROMQL 보고서 타입
* `OpenMetrics 대시보드`의 월간 지표 보고서를 생성합니다.
* 지표는 PROMQL을 사용합니다. 이는 `OpenMetrics 대시보드`의 차트에서 쓰이는 것과 동일하며, 보다 자세한 예시는 [https://prometheus.io/docs/prometheus/latest/querying/basics/](https://prometheus.io/docs/prometheus/latest/querying/basics/)에서 확인 가능합니다.
* 주기는 `5분`, `30분`, `2시간`, `1일`중 하나를 선택할 수 있으며, 선택한 주기에 따라 `월`의 선택이 제한됩니다.(최대 6개월 전 까지 선택 가능)
* `다운로드 요청`버튼을 누르면 보고서를 생성 요청하게 되며 이 목록은 `보고서 생성 요청 현황`에서 확인 가능합니다.
   * 보고서의 생성은 비동기적으로 이루어지므로 생성이 완료될 때 까지는 다운로드할 수 없습니다.
